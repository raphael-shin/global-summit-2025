...
class ByocGfpganCodeBuildStack(Stack):
    def __init__(self, scope: Construct, id: str, repository: ecr.IRepository, **kwargs) -> None:
        ...
        # Store the repository in the stack
        self.repository = repository

        # Get the absolute path of the project root
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        gfpgan_dir = os.path.join(project_root, "byoc", "gfpgan")

        # Verify if the directory exists
        if not os.path.exists(gfpgan_dir):
            raise ValueError(f"Directory not found: {gfpgan_dir}")

        # Create an S3 bucket to store the Dockerfile and source files
        bucket = s3.Bucket(self, "ByocGfpganSourceBucket",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True)

        # Upload the Dockerfile and src directory to the S3 bucket
        s3_deployment = s3deploy.BucketDeployment(self, "DeployByocGfpganDockerfileAndSrc",
            sources=[s3deploy.Source.asset(gfpgan_dir)],
            destination_bucket=bucket,
            destination_key_prefix="source")

        # Create a CodeBuild project
        self.project = codebuild.Project(self, "ByocGfpganBuildProject",
            project_name="ByocGfpganBuildProject",
            description="Build Docker image for BYOC GFPGAN",
            ...
            environment_variables={
                ...
                "ECR_REPOSITORY": codebuild.BuildEnvironmentVariable(value="pytorch-inference"),
                "IMAGE_TAG": codebuild.BuildEnvironmentVariable(value="2.3.0-gpu-py311-cu121-ubuntu20.04-ec2")
            },
        ...
        CfnOutput(self, "GfpganCodeBuildProjectName", value=self.project.project_name)
